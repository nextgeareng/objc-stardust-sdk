...
#import "NGSJSONRequestSerializer.h"
#import "NGSQueryParamCollection.h"
#import "NGSDefaultConfiguration.h"

NSString *const NGSResponseObjectErrorKey = @"NGSResponseObject";

static NSString * const kNGSContentDispositionKey = @"Content-Disposition";

static NSDictionary * NGS__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * NGS__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = NGS__headerFieldsForResponse(response);
    if(!headers[kNGSContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kNGSContentDispositionKey];
...
@interface NGSApiClient ()

@property (nonatomic, strong, readwrite) id<NGSConfiguration> configuration;

@property (nonatomic, strong) NSArray<NSString*>* downloadTaskResponseTypes;

@end

@implementation NGSApiClient

#pragma mark - Singleton Methods

+ (instancetype) sharedClient {
    static NGSApiClient *sharedClient = nil;
...
- (instancetype)init {
    return [self initWithConfiguration:[NGSDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithBaseURL:(NSURL *)url {
    return [self initWithBaseURL:url configuration:[NGSDefaultConfiguration sharedConfig]];
}
...
- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<NGSConfiguration>)configuration {
    ...
    if (self) {
        ...
        _timeoutInterval = 60;
        _responseDeserializer = [[NGSResponseDeserializer alloc] init];
        _sanitizer = [[NGSSanitizer alloc] init];

        _downloadTaskResponseTypes = @[@"NSURL*", @"NSURL"];

        AFHTTPRequestSerializer* afhttpRequestSerializer = [AFHTTPRequestSerializer serializer];
        NGSJSONRequestSerializer * swgjsonRequestSerializer = [NGSJSONRequestSerializer serializer];
        _requestSerializerForContentType = @{kNGSApplicationJSONType : swgjsonRequestSerializer,
...
- (NSURLSessionDataTask*) taskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask *task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        NGSDebugLogResponse(response, responseObject,request,error);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[NGSResponseObjectErrorKey] = responseObject;
        }
...
- (NSURLSessionDataTask*) downloadTaskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask* task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString *filename = NGS__fileNameForResponse(response);
...
- (NSURLSessionTask*) requestWithPath: (NSString*) path
    ...
    __weak id<NGSSanitizer> sanitizer = self.sanitizer;
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = NGSPercentEscapedStringFromString(safeString);
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path queryParams:(NSDictionary*) queryParams {
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = NGSPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, NGSPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[NGSQueryParamCollection class]]){
            NGSQueryParamCollection * coll = (NGSQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = NGSPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = NGSPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = NGSPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (void) updateHeaderParams:(NSDictionary * *)headers queryParams:(NSDictionary * *)querys WithAuthSettings:(NSArray *)authSettings {
    ...
    id<NGSConfiguration> config = self.configuration;
