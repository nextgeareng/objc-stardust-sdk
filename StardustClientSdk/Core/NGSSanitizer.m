...
NSString * const kNGSApplicationJSONType = @"application/json";

NSString * NGSPercentEscapedStringFromString(NSString *string) {
    static NSString * const kNGSCharactersGeneralDelimitersToEncode = @":#[]@";
    static NSString * const kNGSCharactersSubDelimitersToEncode = @"!$&'()*+,;=";

    NSMutableCharacterSet * allowedCharacterSet = [[NSCharacterSet URLQueryAllowedCharacterSet] mutableCopy];
    [allowedCharacterSet removeCharactersInString:[kNGSCharactersGeneralDelimitersToEncode stringByAppendingString:kNGSCharactersSubDelimitersToEncode]];
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[NGSQueryParamCollection class]]) {
        return object;
    }
    else if ([object isKindOfClass:[NSDate class]]) {
        return [NGSSanitizer dateToString:object];
    }
    ...
    else if ([object isKindOfClass:[NGSObject class]]) {
...
- (NSString *) parameterToString:(id)param {
    if ([param isKindOfClass:[NSString class]]) {
        ...
        return [NGSSanitizer dateToString:param];
    }
...
+ (NSString *)dateToString:(id)date {
    NSTimeZone* timeZone = [NGSDefaultConfiguration sharedConfig].serializationTimeZone;
...
- (NSString *) selectHeaderAccept:(NSArray *)accepts {
    ...
    for (NSString *string in accepts) {
        if ([self.jsonHeaderTypeExpression matchesInString:string options:0 range:NSMakeRange(0, [string length])].count > 0) {
            return kNGSApplicationJSONType;
        }
